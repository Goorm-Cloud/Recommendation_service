pipeline {
    ...
    environment {
        ...
        GITWEBADD = 'https://github.com/Goorm-Cloud/Recommendation_service.git'
        GITSSHADD = 'git@github.com:Goorm-Cloud/manifest-recommendation.git'
        GITCREDENTIAL = 'git_cre_zinucha'
        ECR_REGISTRY = '651706756261.dkr.ecr.ap-northeast-2.amazonaws.com'
        ECR_REPO = 'recommendation_service'
        ...
        stage('Update EKS Manifest') {
            steps {
                script {
                    ...
                    // 이미지 태그 변경 (빌드 번호 적용)
                    sh "sed -i 's@image:.*@image: ${ECR_REGISTRY}/${ECR_REPO}:${currentBuild.number}@g' recommendation-service.yaml"
